    ...
    <div className="min-h-screen bg-black text-white pt-[72px]">
      <div className="max-w-[1400px] mx-auto px-6 py-12">
        ...
        <div className="mt-10 flex justify-between items-center border-y border-zinc-900 py-4">
          ...
          <input placeholder="SEARCH SOHO, CENTRAL..." className="w-[280px] h-[36px] rounded-full bg-zinc-900 border border-zinc-800 px-5 text-[11px] tracking-widest placeholder:text-zinc-600 outline-none"/>
        </div>

        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          {VENUES_DATA.map(v=>(
            <div key={v.id} className="bg-neutral-900 border border-zinc-800 rounded-[20px] p-3 group hover:border-zinc-700 transition-colors">
              <div className="relative">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10 px-3 h-[24px] rounded-full bg-white text-black text-[10px] tracking-widest flex items-center gap-1 font-medium"><span className="w-4 h-4 rounded-full bg-black text-white flex items-center justify-center text-[8px]">{v.letter}</span>{v.host}</div>
                <div className="relative overflow-hidden rounded-[16px] h-[320px]">
                  <img src={v.img} alt={v.name} className="w-full h-full object-cover"/>
                  <div className="absolute top-3 left-3 px-3 py-1 rounded-full bg-black backdrop-blur text-[10px] tracking-widest">{v.area} · {v.time}</div>
                  <div className="absolute bottom-3 left-3 px-3 py-1 rounded-full bg-orange-700 text-white text-[10px] tracking-widest">{v.spots}</div>
                </div>
              </div>
              <div className="p-3">
                ...
                <div className="mt-3 text-[11px] text-zinc-500 leading-relaxed group-hover:text-orange-600 transition-colors">{v.invite}</div>
                <div className="mt-3 text-[10px] tracking-widest text-zinc-600">{v.meta}</div>
                <div className="mt-4 flex gap-2">
                  <button onClick={()=>openJoin(v)} className="flex-1 h-[40px] rounded-full bg-white text-black text-[11px] tracking-widest font-medium">JOIN</button>
                  <button onClick={()=>{
                    if(typeof window!=="undefined"){
                      const isRegistered = localStorage.getItem("buddy_registered")==="1";
