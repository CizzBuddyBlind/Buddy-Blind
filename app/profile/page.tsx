    ...
    <main className="bg-[#080808] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-6 pt-10 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8">
          ...
          <div className="bg-[#0f0f0f] border border-zinc-900 rounded-2xl p-6 md:p-8 h-fit sticky top-20">
            <div className="relative">
              ...
              <div className="absolute -top-2 left-1/2 -translate-x-1/2 -translate-y-1 ml-8 mono text-xs tracking-[0.1em] bg-[#f5f2eb] text-black px-3 py-1 rounded-full">{profile.tier}</div>
            </div>

            {!editMode ? (
              <>
                <h1 className="mt-8 serif text-[36px] text-white text-center">{profile.name}</h1>
                <div className="mt-2 mono text-xs tracking-[0.15em] text-zinc-500 text-center">{buddies} BUDDIES · {profile.rating} ★</div>

                <div className="mt-6 flex flex-wrap gap-2 justify-center">
                  <span className="mono text-xs tracking-[0.1em] bg-[#1a1a1a] border border-zinc-800 px-3 py-1.5 rounded-full text-zinc-400">{profile.gender}</span>
                  <span className="mono text-xs tracking-[0.1em] bg-[#1a1a1a] border border-zinc-800 px-3 py-1.5 rounded-full text-zinc-400">{profile.role}</span>
                  <span className="mono text-xs tracking-[0.1em] bg-[#1a1a1a] border border-zinc-800 px-3 py-1.5 rounded-full text-zinc-400">{profile.location}</span>
                  <span className="mono text-xs tracking-[0.1em] bg-[#1a1a1a] border border-zinc-800 px-3 py-1.5 rounded-full text-zinc-400">{profile.age_range}</span>
                  <span className="mono text-xs tracking-[0.1em] bg-[#1a1a1a] border border-zinc-800 px-3 py-1.5 rounded-full text-zinc-400">{profile.orientation}</span>
                </div>

                <button onClick={()=>{setForm(profile); setEditMode(true)}} className="mt-6 w-full h-10 rounded-full border border-zinc-800 mono text-xs tracking-[0.1em] text-zinc-400 hover:border-zinc-600">EDIT PROFILE</button>

                <div className="mt-8 border-t border-zinc-900 pt-6">
                  <div className="mono text-xs tracking-[0.15em] text-zinc-600" style={getStyle('profile_points_label')}>{getText('profile_points_label')}</div>
                  <div className="mt-4 flex gap-2">
                    {['BRONZE','SILVER','GOLD'].map(t=><button key={t} className={`mono text-xs tracking-[0.1em] px-3 py-1.5 rounded-full border transition ${profile.tier===t?'bg-white text-black border-white':'border-zinc-800 text-zinc-600'}`}>{t}</button>)}
                  </div>
                  <div className="mt-4 mono text-xs tracking-[0.05em] text-zinc-500" style={getStyle('profile_points_desc')}>{getText('profile_points_desc')}</div>
                  ...
                  <div className="mt-2 flex justify-between mono text-xs text-zinc-600">
                  ...
                  <div className="mt-4 space-y-1 mono text-xs text-zinc-600">
              ...
              <div className="mt-6 space-y-4">
                <h2 className="mono text-xs tracking-[0.15em] text-white">EDIT YOUR PROFILE - User can edit name, where they live, not full story</h2>
                <div>
                  <label className="mono text-xs text-zinc-500">NAME (editable)</label>
                ...
                <div>
                  <label className="mono text-xs text-zinc-500">WHERE YOU LIVE (editable) *required</label>
                ...
                <div>
                  <label className="mono text-xs text-zinc-500">GENDER - select male/female *required</label>
                ...
                <div>
                  <label className="mono text-xs text-zinc-500">AGE RANGE - 20-25, 25-30 etc *required</label>
                ...
                <div>
                  <label className="mono text-xs text-zinc-500">SEX ORIENTATION - gay, straight, lesbian, bi, trans *required for filtering</label>
                  ...
                  <div className="mono text-xs text-zinc-600 mt-1">Required because events like lesbian-only dinner auto-filter: only female + lesbian/bi allowed</div>
                </div>
                <div>
                  <label className="mono text-xs text-zinc-500">ROLE / INDUSTRY</label>
                ...
                <div className="flex gap-2 mt-4">
                  <button onClick={saveProfile} className="flex-1 h-11 rounded-full bg-white text-black mono text-xs tracking-[0.1em]">SAVE</button>
                  <button onClick={()=>setEditMode(false)} className="flex-1 h-11 rounded-full border border-zinc-800 mono text-xs tracking-[0.1em] text-zinc-500">CANCEL</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div>
              ...
              <div className="mt-6 bg-[#0f0f0f] border border-zinc-900 rounded-2xl p-8 flex items-center gap-6">
                <div className="w-20 h-20 rounded-full bg-[#1a1a1a] border border-zinc-800 flex items-center justify-center">
                  <span className="mono text-2xl text-white">👥</span>
                </div>
                <div>
                  <div className="serif text-[48px] text-white leading-none">{buddies}</div>
                  <div className="mono text-xs tracking-[0.15em] text-zinc-500 mt-1">BUDDIES</div>
                  <div className="mono text-xs text-zinc-600 mt-1">Auto-counted when other accepted to be buddy. Per Bible: Be My Buddies.</div>
                </div>
                <div className="ml-auto">
                  <Link href="/venues" className="mono h-10 px-5 rounded-full bg-[#f5f2eb] text-black text-xs tracking-[0.1em] flex items-center hover:bg-white">FIND BUDDIES</Link>
                </div>
              </div>
            </div>
            ...
            <div className="mt-12 border-t border-zinc-900 pt-8">
              <div className="flex justify-between items-center">
                ...
                <button className="mono h-9 px-4 rounded-full border border-zinc-800 text-xs tracking-[0.1em] text-zinc-400 hover:border-zinc-600" style={getStyle('profile_rate_btn')}>{getText('profile_rate_btn')}</button>
              </div>

              <div className="mt-6 space-y-4">
                {comments.map((c,i)=>(
                  <div key={i} className="bg-[#0f0f0f] border border-zinc-900 rounded-2xl p-5">
                    <div className="flex justify-between items-center">
                      <span className="mono text-xs tracking-[0.1em] text-zinc-400">{c.user}</span>
                      <span className="mono text-xs text-[#c96a4a]">{'★'.repeat(c.stars)}{'☆'.repeat(5-c.stars)}</span>
                    </div>
                    <p className="mt-3 text-sm leading-relaxed text-zinc-300">{c.text}</p>
                    <div className="mt-2 mono text-xs text-zinc-600">Only users who attended same event can comment. Per Bible.</div>
                  </div>
                ))}
              </div>

              <div className="mt-8 bg-[#0f0f0f] border border-zinc-900 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <div className="mono text-xs tracking-[0.15em] text-zinc-600" style={getStyle('profile_vibe_label')}>{getText('profile_vibe_label')}</div>
                  <div className="serif mt-2 text-xl text-white" style={getStyle('profile_vibe_quote')}>{getText('profile_vibe_quote')}</div>
                </div>
                <button onClick={handleBeBuddy} className="mono h-11 px-6 rounded-full bg-[#f5f2eb] text-black text-xs tracking-[0.15em] hover:bg-white transition shrink-0" style={getStyle('profile_vibe_btn')}>{getText('profile_vibe_btn')}</button>
              </div>
            </div>

            <div className="mt-12 mono text-xs tracking-[0.1em] text-zinc-700" style={getStyle('profile_footer')}>{getText('profile_footer')}</div>
          </div>
        </div>
      </div>
    </main>
