  ...
  return(
    <main className="bg-[#080808] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-6 pt-10 pb-20 grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-10">
        <div className="pt-8">
          <EditableText textKey="top_stats" defaultValue={getText('top_stats')} as="div" className="mono text-xs tracking-[0.15em] text-zinc-500" style={getStyle('top_stats')} />
          ...
          <div className="mt-12 text-sm leading-relaxed text-zinc-400 max-w-md">
          ...
          <div className="mt-10 flex gap-3">
            <EditableText textKey="cta_join" defaultValue={getText('cta_join')} as="div" className="mono h-12 px-7 rounded-full bg-[#f5f2eb] text-black text-xs tracking-[0.15em] flex items-center hover:bg-white transition" style={getStyle('cta_join')}>
              <Link href="/venues" className="w-full h-full flex items-center justify-center">{getText('cta_join')}</Link>
            </EditableText>
            <EditableText textKey="cta_private" defaultValue={getText('cta_private')} as="div" className="mono h-12 px-7 rounded-full border border-zinc-800 text-xs tracking-[0.15em] flex items-center text-zinc-400 hover:border-zinc-600 hover:text-zinc-200 transition" style={getStyle('cta_private')}>
              <Link href="/private-events" className="w-full h-full flex items-center justify-center">{getText('cta_private')}</Link>
            </EditableText>
          </div>
          <div className="mt-24 border-t border-zinc-900 pt-8 grid grid-cols-3 gap-8 max-w-md">
            <div><EditableText textKey="stat1_num" defaultValue={getText('stat1_num')} as="div" className="serif text-[36px] text-white" style={getStyle('stat1_num')} /><EditableText textKey="stat1_label" defaultValue={getText('stat1_label')} as="div" className="mono mt-2 text-xs tracking-[0.1em] text-zinc-500 leading-relaxed" style={getStyle('stat1_label')} /></div>
            <div><EditableText textKey="stat2_num" defaultValue={getText('stat2_num')} as="div" className="serif text-[36px] text-white" style={getStyle('stat2_num')} /><EditableText textKey="stat2_label" defaultValue={getText('stat2_label')} as="div" className="mono mt-2 text-xs tracking-[0.1em] text-zinc-500" style={getStyle('stat2_label')} /></div>
            <div><EditableText textKey="stat3_num" defaultValue={getText('stat3_num')} as="div" className="serif text-[36px] text-white" style={getStyle('stat3_num')} /><EditableText textKey="stat3_label" defaultValue={getText('stat3_label')} as="div" className="mono mt-2 text-xs tracking-[0.1em] text-zinc-500" style={getStyle('stat3_label')} /></div>
          </div>
        </div>
        <div className="lg:pt-4">
          <EditableText textKey="featured_label" defaultValue={getText('featured_label')} as="div" className="flex items-center gap-2 mono text-xs tracking-[0.15em] text-zinc-500" style={getStyle('featured_label')} />
          <div className="mt-8 bg-[#121212] border border-zinc-900 rounded-[24px] relative">
            <div className="absolute -top-3 right-5 z-20 flex items-center gap-2 bg-[#f5f2eb] text-black mono text-xs tracking-[0.1em] px-4 py-2 rounded-full border border-black/10 shadow-xl">
              <div className="w-5 h-5 rounded-full bg-black text-white flex items-center justify-center text-xs">C</div>
              {featured.host_label}
            </div>
            ...
            <div className="p-7">
              ...
              <EditableText textKey="featured_vibe" defaultValue={featured.vibe_label} as="div" className="mt-2 mono text-xs tracking-[0.1em] text-zinc-500 leading-relaxed" />
              <EditableText textKey="featured_invite" defaultValue={featured.invite_text} as="div" className="mt-4 mono text-xs tracking-[0.05em] text-zinc-400 leading-relaxed" />
              <EditableText textKey="featured_desc" defaultValue={featured.description_long} as="p" className="mt-6 text-sm leading-relaxed text-zinc-400" />
              <div className="mt-8 flex gap-3">
                <Link href={`/join?venue=${featured.id}`} className="flex-1 mono h-12 rounded-full bg-[#f5f2eb] text-black text-xs tracking-[0.15em] flex items-center justify-center hover:bg-white transition">JOIN BLIND BOX</Link>
                <Link href="/invite" className="mono h-12 px-6 rounded-full border border-zinc-800 text-xs tracking-[0.15em] flex items-center justify-center text-zinc-300 hover:border-zinc-600 transition">INVITE</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      <footer className="border-t border-zinc-900 mt-10 py-6">
        <div className="max-w-[1400px] mx-auto px-6 flex justify-between items-center">
          <EditableText textKey="footer_left" defaultValue={getText('footer_left')} as="div" className="mono text-xs tracking-[0.1em] text-zinc-600" style={getStyle('footer_left')} />
          <div className="flex gap-6 mono text-xs tracking-[0.1em] text-zinc-600"><Link href="/how-it-works" className="hover:text-zinc-300">How it works</Link><Link href="/premium" className="hover:text-zinc-300">Premium</Link></div>
        </div>
      </footer>
    </main>
  )
