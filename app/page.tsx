...
type Featured = {
  id:string, title:string, area:string, time:string, spots_left:number, host_label:string, price_label:string, vibe_label:string, description_short:string, description_long:string, invite_text:string, image_url:string
}
const DEFAULT_FEATURED:Featured = {
  id:'1', title:'Kissa Tanaka', area:'SOHO', time:'TONIGHT 7:30PM', spots_left:3, host_label:'HOST: COMEDIAN · GOLD', price_label:'$$ · CREATIVE MINDS', vibe_label:'SOHO · KISSATEN · HOST CREATES ATTRACTION AND DOWNLOAD REASONS', description_short:'CJ INVITES YOU TO JOIN A DINNER AND MEET NEW FRIENDS - NO PITCHES, JUST PRESENCE.', description_long:'A 6-seat counter, vinyl crackle, no menus. You order by mood. Tonight is for people who collect stories, not contacts. No pitches, just presence.', invite_text:'CJ INVITES YOU TO JOIN A DINNER AND MEET NEW FRIENDS - NO PITCHES, JUST PRESENCE.', image_url:'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=1200'
}
    ...
    <main className="bg-[#080808] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-6 pt-10 pb-20 grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-10">
        ...
        <div className="pt-8">
          ...
          <div className="mt-10 flex gap-3">
            <a href="/venues" className="mono h-[48px] px-7 rounded-full bg-[#f5f2eb] text-black text-[11px] tracking-[0.15em] flex items-center hover:bg-white transition">{content.cta_join}</a>
            <a href="/private-events" className="mono h-[48px] px-7 rounded-full border border-zinc-800 text-[11px] tracking-[0.15em] flex items-center text-zinc-400 hover:border-zinc-600 hover:text-zinc-200 transition">{content.cta_private}</a>
          </div>
        ...
        <div className="lg:pt-4">
          ...
          <div className="mt-4 bg-[#121212] border border-zinc-900 rounded-[24px] relative">
            ...
            <div className="p-7">
              ...
              <button className="mt-4 mono text-[10px] tracking-[0.15em] border border-zinc-800 px-4 py-2 rounded-full text-white hover:border-zinc-600 transition">SHARE</button>
              <p className="mt-6 text-[13px] leading-relaxed text-zinc-400">{featured.description_long}</p>
              <div className="mt-8 flex gap-3">
                <a href={`/join?venue=${featured.id}`} className="flex-1 mono h-[48px] rounded-full bg-[#f5f2eb] text-black text-[11px] tracking-[0.15em] flex items-center justify-center hover:bg-white transition">JOIN BLIND BOX</a>
                <a href="/invite" className="mono h-[48px] px-6 rounded-full border border-zinc-800 text-[11px] tracking-[0.15em] flex items-center justify-center text-zinc-300 hover:border-zinc-600 transition">INVITE</a>
              </div>
      ...
      <footer className="border-t border-zinc-900 mt-10 py-6">
        <div className="max-w-[1400px] mx-auto px-6 flex justify-between items-center">
          ...
          <div className="flex gap-6 mono text-[10px] tracking-[0.1em] text-zinc-600"><a href="/how-it-works" className="hover:text-zinc-300">How it works</a><a href="/premium" className="hover:text-zinc-300">Premium</a></div>
        </div>
      </footer>
    </main>
  )
