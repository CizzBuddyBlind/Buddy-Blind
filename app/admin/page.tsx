  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white p-8">
      <div className="max-w-[800px] mx-auto">
        ...
        <p className="mt-2 text-[13px] text-zinc-500">Edit mode only affects main pages (Home text). Join / Invite flow modals are protected and will NOT show edit pencils, so V9 flow stays simple & stylish.</p>
        <div className="mt-8 bg-[#111] border border-zinc-800 rounded-[20px] p-6 flex justify-between items-center">
          <div><div className="text-[12px] tracking-[0.2em]">EDIT MODE</div><div className="mt-1 text-[11px] text-zinc-500">{isAdmin ? 'ON - Home page shows ✎ pencils, you can change text, color, move sections, upload photo' : 'OFF - Site shows clean V9 design'}</div></div>
          <button onClick={toggle} className={`px-6 py-2 rounded-full font-bold text-[12px] ${isAdmin?'bg-amber-400 text-black':'bg-white text-black'}`}>{isAdmin?'DISABLE':'ENABLE'}</button>
        </div>
        <div className="mt-6 text-[12px] text-zinc-600 space-y-2">
