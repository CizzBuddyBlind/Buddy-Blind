...
const PRIVATE_DATA = [
  ...
  { id:"plating", title:"Plating Together", desc:"Private kitchen where strangers plate each others dish", host:"HOST: CHEF LIN · 6 SEATS", attraction:"Attraction: You cook for someone you havent met. They do the same.", badge:"15 TABLES", img:"https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=800" },
    ...
    <div className="min-h-screen bg-black text-white pt-[72px]">
      <div className="max-w-[1400px] mx-auto px-6 py-12">
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
          {PRIVATE_DATA.map(v=>(
            <div key={v.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3 group hover:border-zinc-700 transition-colors">
              <div className="relative overflow-hidden rounded-2xl h-[320px]">
                <img src={v.img} alt={v.title} className="w-full h-full object-cover"/>
                <div className="absolute top-3 left-3 px-3 py-1 rounded-full bg-black backdrop-blur text-xs tracking-widest">{v.badge}</div>
              </div>
              <div className="p-3">
                ...
                <div className="mt-3 text-xs text-zinc-500 italic leading-relaxed group-hover:text-[#C45A3C] transition-colors">“{v.attraction}”</div>
                <div className="mt-4 flex gap-2">
                  <button onClick={()=>openJoin(v)} className="flex-1 h-10 rounded-full bg-white text-black text-xs tracking-widest font-medium">JOIN</button>
                  <button onClick={()=>openJoin(v)} className="px-5 h-10 rounded-full border border-zinc-800 text-xs tracking-widest">INVITE</button>
                </div>
              </div>
            </div>
          ))}
