      ...
      <div className="mt-12 w-full max-w-2xl bg-[#111] border border-zinc-800 rounded-[32px] p-8">
        {!paid ? (
          <>
            ...
            <p className="mt-4 text-sm text-zinc-400">Pay $5 to create a private blind dinner. 1 invite = 1 buddy. Host creates attraction.</p>
            <button onClick={handleCheckout} className="mt-6 w-full h-14 rounded-full bg-white text-black font-black text-xs tracking-widest">PAY $5 TO UNLOCK INVITE LINK</button>
            <p className="mt-4 text-center text-[11px] text-zinc-600">Test card: 4242 4242 4242 4242 | 12/34 | 123</p>
          </>
        ) : (
          <>
            ...
            <button onClick={copy} className="mt-6 w-full h-14 rounded-full bg-white text-black font-black text-xs tracking-widest">{copied ? 'COPIED ✓' : 'COPY LINK'}</button>
