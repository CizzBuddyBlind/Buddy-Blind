...
export default function Join(){
  ...
  const book=async()=>{
    ...
    if(current && current.places_left>0){await supabase.from('venues').update({places_left: current.places_left-1}).eq('id',venueId); alert(`Booked ${venue?.name}! Check email ${email}`)}
    else alert('Sold out')
  }
