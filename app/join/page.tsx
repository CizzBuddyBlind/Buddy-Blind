...
export default function JoinPage() {
  ...
  useEffect(() => {
    // After Stripe $5 admin fee success, decrement places_left
  ...
  const handleBook = async () => {
    ...
    setLoading(true);
    // Trigger $5 admin fee checkout per Features Bible
  ...
  if (booked) {
    return (
      <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center px-6">
        ...
        <p className="mt-4 text-zinc-500 text-sm text-center">You paid $5 admin fee. {venue?.name || 'Blind dinner'} secured. Address revealed 2h before. Ref: {ref}</p>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white px-6 py-20 max-w-xl mx-auto">
      ...
      <div className="mt-8 space-y-4">
        <input value={email} onChange={e => setEmail(e.target.value)} placeholder="your email" className="w-full h-14 rounded-full bg-[#111] border border-zinc-800 px-6 text-sm" />
