...
export function EditableImage({ imageKey, defaultSrc, alt='', className='', style={} }: Props){
  ...
  const saveSrc = (ns: string) => {
    setSrc(ns)
    try{ const d=JSON.parse(localStorage.getItem('bb_draft')||'{}'); if(!d.siteContent) d.siteContent={}; d.siteContent[imageKey]=ns; localStorage.setItem('bb_draft', JSON.stringify(d)); localStorage.setItem('bb_has_unsaved','1'); window.dispatchEvent(new Event('storage')) }catch{}
    setShowEdit(false)
  }
