...
export default function RegisterFlow({isOpen,onClose,onComplete,venueName}:Props){
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-80 flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-zinc-900 border border-zinc-800 rounded-3xl p-8 max-h-screen overflow-y-auto">
        ...
        {step===1 && (
          <div className="mt-6 space-y-4">
            <div className="text-xs text-zinc-500">STEP 1 IDENTITY Gender Age 5yr gap Orientation Optional Phone MUST</div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <input value={form.exactAge} onChange={e=>{ const v=e.target.value; const n=parseInt(v); setForm({...form,exactAge:v,ageRange:isNaN(n)?form.ageRange:getRange(n)}); }} placeholder="Age e.g. 18" type="number" className="w-full h-12 rounded-xl bg-black border border-zinc-800 px-4 text-sm outline-none"/>
              <div className="h-12 rounded-xl bg-zinc-800 border border-zinc-700 px-4 flex items-center text-sm">Auto <span className="ml-2 font-bold">{form.ageRange}</span></div>
            </div>
            <div className="flex gap-2">
              <input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="Phone MUST for OTP" className="flex-1 h-12 rounded-xl bg-black border border-amber-500 px-4 text-sm outline-none"/>
            ...
            <input value={form.password} onChange={e=>setForm({...form,password:e.target.value})} type="password" placeholder="Password min 6" className="w-full h-12 rounded-xl bg-black border border-zinc-800 px-4 text-sm outline-none"/>
            <input value={form.confirmPassword} onChange={e=>setForm({...form,confirmPassword:e.target.value})} type="password" placeholder="Confirm retype same" className="w-full h-12 rounded-xl bg-black border border-zinc-800 px-4 text-sm outline-none"/>
            <input value={form.otp} onChange={e=>setForm({...form,otp:e.target.value})} placeholder="OTP 123456" className="w-full h-12 rounded-xl bg-black border border-amber-500 px-4 text-sm text-center outline-none"/>
            <label className="flex gap-2 text-xs text-zinc-400"><input type="checkbox" checked={form.agreeTerms} onChange={e=>setForm({...form,agreeTerms:e.target.checked})}/>I agree to Terms and Conditions</label>
            <button disabled={!form.firstName||!form.email||!form.phone} onClick={()=>setStep(2)} className="w-full h-12 rounded-full bg-violet-600 text-white font-black text-xs disabled:opacity-30">Next Free Trial</button>
          </div>
        )}
        {step===2 && (
          <div className="mt-6">
            <div className="bg-black border border-zinc-800 rounded-2xl p-5 space-y-3">
              <div className="text-sm font-bold">Featured in 100plus countries BuddyBlind</div>
              <div className="space-y-2 mt-3">
                {['Enjoy first 90 days free','Cancel from app or iCloud','Quick match and blind dinners','Detailed venue info','90 days Free Trial','Only 50 per month after'].map((t,i)=>(
                  <div key={i} className="flex gap-2 items-center text-xs"><div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-black">v</div>{t}</div>
                ))}
              </div>
            </div>
            <div className="mt-6 bg-zinc-800 rounded-2xl p-4">
              ...
              <button onClick={()=>setStep(3)} className="mt-4 w-full h-14 rounded-full bg-white text-black font-black">Continue</button>
            </div>
            <button onClick={()=>setStep(1)} className="mt-4 w-full h-11 rounded-full border border-zinc-700 text-xs">BACK</button>
          </div>
        )}
        {step===3 && (
          <div className="mt-6">
            <div className="bg-black border border-zinc-800 rounded-2xl p-5 space-y-4">
              <div className="flex justify-between"><span className="text-xs text-zinc-500">PREMIUM PLAN</span><span className="text-xl font-black">50 per mo</span></div>
              <div className="bg-zinc-900 border border-amber-500 rounded-xl p-3 text-xs font-mono">4242 4242 4242 4242 Exp 12 34 CVC 123 Test User OTP 123456</div>
              <div className="text-xs text-zinc-500">90-DAY FREE 5 admin per join plus 88 deposit at venue Blind dining address 2h before 18plus respectful Photos coloured different per event</div>
            </div>
            <label className="mt-4 flex gap-2 text-xs bg-zinc-800 rounded-xl p-3 border border-zinc-700"><input type="checkbox" checked={form.agreeTrial} onChange={e=>setForm({...form,agreeTrial:e.target.checked})}/>I agree 90-DAY FREE then 50 per mo saved Visa 4242 for 1-click</label>
