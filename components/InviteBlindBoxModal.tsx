...
export default function InviteBlindBoxModal({isOpen,onClose,onConfirm,venue}:Props){
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black backdrop-blur flex items-center justify-center p-4">
      <div className="w-full max-w-[560px] bg-neutral-900 border border-zinc-800 rounded-[28px] overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-7">
            ...
            <div>
              ...
              <div className="grid grid-cols-7 gap-2">
                {weekDates.map((d,i)=>{
                  ...
                  return (
                    <button key={i} onClick={()=>{setData({...data,dateObj:d, dateLabel: d.toLocaleDateString("en-US",{weekday:"short", month:"short", day:"numeric"})}); setStep(3);}} className={`py-3 rounded-[16px] border flex flex-col items-center ${isSelected?"bg-white text-black border-white":"border-zinc-800 bg-black"} ${isToday?"ring-1 ring-amber-400":""}`}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <button key={tp.id} onClick={()=>{setData({...data,type:tp.id}); setStep(5);}} className={`p-5 rounded-[20px] border text-left ${data.type===tp.id?"bg-white text-black border-white":"border-zinc-800 bg-black"}`}>
          ...
          {step===6 && (
            <div>
              ...
              <button onClick={()=>setStep(7)} className="mt-8 w-full h-[56px] rounded-full bg-white text-black font-bold text-[13px] tracking-widest">CONTINUE -> SUMMARY</button>
            </div>
          )}
          ...
          {step===7 && (
            <div>
              ...
              <div className="bg-black border border-zinc-800 rounded-[16px] p-5 space-y-3 text-[12px]">
              ...
              <button disabled={!data.agree} onClick={()=>onConfirm(data)} className="mt-6 w-full h-[56px] rounded-full bg-orange-700 text-white font-bold text-[13px] tracking-widest disabled:opacity-50">CONFIRM BLIND BOX</button>
            </div>
          )}
        </div>
      </div>
