...
export function eventPhotos(event) {
  const urls = [...(event?.gallery || []), event?.imageUrl, event?.poster].filter((src) => typeof src === "string" && src);
  const live = urls.filter((src) => src.startsWith("http") || src.startsWith("data:") || src.startsWith("blob:"));
  const local = urls.filter((src) => src.startsWith("idb:"));
  return [...new Set(local.length ? local : live)].slice(0, 6);
}

export function eventPoster(event) {
  const urls = [...(event?.gallery || []), event?.imageUrl, event?.poster].filter((src) => typeof src === "string" && src);
  return urls.find((src) => src.startsWith("http") || src.startsWith("data:") || src.startsWith("blob:")) || "";
}
...
export function normalizeContent(raw) {
  ...
  content.events.forEach((event, index) => {
    ...
    if (event.kind === "private") {
      const seeded = (SEED.events || []).find((item) => item.id === event.id);
      const dead = "photo-1611501275019-9b6b9e00d8b0";
      const swap = (src) => (typeof src === "string" && src.includes(dead) && seeded?.imageUrl ? seeded.imageUrl : src);
      event.imageUrl = swap(event.imageUrl);
      if (Array.isArray(event.gallery)) event.gallery = event.gallery.map(swap);
      event.poster = swap(event.poster);
      const urls = [event.imageUrl, ...(event.gallery || [])].filter((src) => typeof src === "string" && src);
      const hasPicture = urls.some((src) => src.startsWith("http") || src.startsWith("data:") || src.startsWith("idb:"));
      if (!hasPicture && seeded?.imageUrl) {
        event.imageUrl = seeded.imageUrl;
        event.gallery = [seeded.imageUrl];
      }
      if (!event.gallery?.filter(Boolean).length && event.imageUrl) event.gallery = [event.imageUrl];
      if (!event.poster && seeded?.imageUrl) event.poster = seeded.imageUrl;
